 ...
 * Botão flutuante do WhatsApp com chamada para ação
 */
const WhatsAppButton = () => {
  ...
  const phoneNumber = "5514332433605"; // (14) 3324-3605
  const message = "Olá! Gostaria de mais informações sobre os serviços jurídicos.";

  const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodeURIComponent(message)}`;

  // Auto-hide tooltip after 10 seconds, then show again every 30 seconds
  useEffect(() => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      ...
      <AnimatePresence>
        {(showTooltip || isHovered) && (
          <motion.div
            ...
            className="relative bg-white rounded-2xl shadow-2xl p-4 max-w-[260px] mr-2"
          >
            {/* Close button */}
            <button
            ...
            <div className="relative z-10">
              ...
              <p className="text-slate-600 text-xs leading-relaxed">
                Fale agora com um de nossos advogados pelo WhatsApp!
              </p>
            </div>
          </motion.div>
        )}
      ...
      >
        ...
        {/* Button */}
        <div className="relative w-16 h-16 bg-green-500 hover:bg-green-600 rounded-full flex items-center justify-center shadow-2xl shadow-green-500/40 transition-all duration-300">
          {/* WhatsApp Icon */}
          <svg
            viewBox="0 0 24 24"
            fill="white"
